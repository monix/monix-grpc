...
class TestServer extends TestServiceGrpcService[Metadata] {
  ...
  override def clientStreaming(request: Observable[Request], ctx: Metadata): Task[Response] =
    request.scanEval(Task(0)){(successCount, req) =>
      req.scenario match {
...
object TestServer {
  ...
  def monixStub(port: Int): TestServiceGrpcService[Metadata] = {
    ...
    val channel = ManagedChannelBuilder.forAddress("localhost", port).build()
